...
def minimal_tsp():
    return {"COMMENT": "", "DIMENSION": None, "TYPE": None, "EDGE_WEIGHT_TYPE": None, "CITIES": []}
...
def breedPopulation(matingpool, eliteSize, crossover):
    children = []
    ...
    for i in range(0, length):
        if crossover == 1:
            ...
            child = uniform(pool[i], pool[len(matingpool)-i-1])
...
def nextGeneration(currentGen, eliteSize, mutationRate, crossover):
    popRanked = rankRoutes(currentGen)
    selectionResults = selection(popRanked, eliteSize)
    matingpool = matingPool(currentGen, selectionResults)
    children = breedPopulation(matingpool, eliteSize, crossover)
    nextGeneration = mutatePopulation(children, mutationRate)
    return nextGeneration

def geneticAlgorithm(population, popSize, eliteSize, mutationRate, generations, crossover):
    pop = initialPopulation(popSize, population)
    print("Initial distance: " + str(1 / rankRoutes(pop)[0][1]))

    for i in range(0, generations):
        pop = nextGeneration(pop, eliteSize, mutationRate, crossover)
...
print('==== Metoda 1 ====')
route1 = geneticAlgorithm(population=cityList, popSize=100, eliteSize=20, mutationRate=0.01, generations=500)
print(route1)
print('\n')
